(function($) { // Avoid conflicts with other libraries
    ...
    function addPost(post, front)
    {
        ...
        $(element).find("[data-type='date']").html(post.date.substring(post.date.length-5));
...
function addForumNofragTribuneTime(heure) {
    //Get Input message
    let messageValue = $("#text_shoutbox").val();

    //Remove hour if already present
    if(messageValue.length > 4 ){
      let potentialDate = messageValue.substring(0, 5);
      let hourRegex = /([01]\d|2[0-3]):([0-5]\d)/;
      if(potentialDate.match(hourRegex)){
        messageValue = messageValue.substring(6);
      }
    }

    //Set date to message
    $("#text_shoutbox").val(heure + ' ' + messageValue);
}
